...
USER_PROMPT = """
[1단계: 개인정보 수집]
사용자가 처음 입력한 이름, 나이, 성별(M/F)을 추출하세요.

규칙:
- 이름은 그대로 출력
- 나이는 정수만 출력
- 성별은 '남자' → 'M', '여자' → 'F', 그 외 → 'Others'
- 반드시 JSON 형식으로 출력하세요.

예시:
{"name": "이하연", "age": 24, "gender": "F"}
""" + EXTRACT_SUFFIX

EMOTION_JSON_PROMPT = """
[감정 분석 JSON 추출]
아래 상담 내용을 보고, 사용자가 언급한 감정을 '상담 전'과 '상담 후'로 반드시 추출하세요.

규칙:
- 반드시 JSON 배열 형식으로 출력합니다.
- 각 항목은 {"emotion_name": 감정명, "emotion_score": 숫자, "division": "전" 또는 "후"} 형태로 작성하세요.
- '전' 그룹과 '후' 그룹 모두 최소 1개 이상의 감정을 반드시 포함해야 합니다.
- 감정 점수는 0~100 사이 정수로 분배하고, 그룹별 총합은 각각 정확히 100이 되도록 하세요.
- 사용자가 감정을 구체적으로 언급하지 않았더라도, 대화 맥락을 바탕으로 일반적인 감정을 가정해서라도 채워 넣으세요.
- 사용자가 상담 후 감정에서 구체적인 언급을 하지않고 변화된 감정에 더해 다른 감정은 비슷하다, 똑같다고 한다면 상담 전에 느낀 감정을 상담 전과 비슷한 비율로 채워 넣으세요.
- 불필요한 설명은 쓰지 말고 JSON만 출력하세요.

예시 출력:
[
  {"emotion_name": "불안", "emotion_score": 40, "division": "전"},
  {"emotion_name": "슬픔", "emotion_score": 60, "division": "전"},
  {"emotion_name": "편안함", "emotion_score": 50, "division": "후"},
  {"emotion_name": "후련함", "emotion_score": 50, "division": "후"}
]
"""
